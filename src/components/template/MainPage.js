...
function MainPage() {
  ...
  return (
    <div className="space-y-8 md:space-y-14">
      <h1 className="text-center text-xl font-semibold text-blue-700 dark:text-white">
      ...
      <div className="space-y-5 tabletz:grid tabletz:grid-cols-2 tabletz:gap-y-10 md:flex md:items-center md:justify-center md:gap-10">
      ...
      <div>
        <h1 className="mb-[2rem] text-center text-base font-semibold text-blue-700 dark:text-white">
          شهرهای پربازدید
        </h1>
        <ul className="grid grid-cols-3 gap-3 text-blue-800">
          {cities.map((item, index) => (
            <li
              key={index}
              className="justify-self-center text-sm dark:text-white"
            >
              {item}
            </li>
          ))}
